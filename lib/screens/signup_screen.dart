...
class _SignUpScreenState extends State<SignUpScreen> {
  ...
  Future<void> _signup() async {
    ...
    if (_dateOfBirth!.isAfter(_maxBirthDate())) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('يجب أن يكون العمر 18 سنة أو أكثر للتبرع')),
      );
      return;
    }
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: const Text('تم إنشاء الحساب بنجاح.'), backgroundColor: primaryRed),
      );
      Navigator.pushReplacementNamed(context, '/home');
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('فشل إنشاء الحساب: $e'), backgroundColor: Colors.redAccent),
      );
    } finally {
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('إنشاء حساب'), backgroundColor: primaryRed),
            ...
            children: [
              ...
              TextFormField(
                ...
                validator: (v) {
                  ...
                  if (int.tryParse(v) == null) return 'رقم الهاتف يجب أن يكون أرقاماً فقط';
                  return null;
                },
              ),
              const SizedBox(height: 15),
              ...
              DropdownButtonFormField<int>(
                decoration: const InputDecoration(labelText: 'فصيلة الدم', border: OutlineInputBorder()),
              ...
              DropdownButtonFormField<String>(
                decoration: const InputDecoration(labelText: 'الجنس (اختياري)', border: OutlineInputBorder()),
                items: _genders
                    .map((g) => DropdownMenuItem(value: g.key, child: Text(g.value)))
                    .toList(),
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  child: _isLoading
                      ...
                      : const Text('إنشاء حساب', style: TextStyle(color: Colors.white, fontSize: 18)),
                ),
              ),
              const SizedBox(height: 20),
            ],
